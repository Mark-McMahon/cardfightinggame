...
## Current state: v1 implemented; Round-6 redesign queued

The v1 monorepo is **built and playable** (M0–M6 — see `README.md`): `shared` engine,
`server` (Colyseus), `client` (React+Vite), and `sim`. The commands and structure below
exist. The **active work** is the Round-6 game-feel + content-depth redesign.

- **`design-spec.md`** — the self-contained, developer-ready build document. **Read this first.** §§1–15 are the implemented v1; **§16 is the Round-6 redesign and supersedes earlier UI/content scoping where they conflict.** Start any client/content work from §16 (build order in §16.8).
- **`requirements.md`** — the decision ledger: 29 locked decisions across 6 rounds, with rationale. Round 6 (#21–29) drives §16. Consult when a design choice is ambiguous; it records *why* each was made.
...
**Redesign gate (design-spec §16.2/§16.7):** the three multipliers (Grovecaller, Echo
Choir, Pale Lich) must get config caps, Pale Lich converts to a breakpoint, and the sim
must gain the reachability metric **before** the new content ships. Validate
content/numbers (build-order steps 1–2) before animating cards (steps 3–4).
...
**Effect data model — hybrid (spec §6).** ~90% of unit effects are pure declarative data: `{trigger, condition?, target, actions[]}` plus passive `AuraSpec`s for multipliers/cost-reduction. The rare ~10% bespoke behaviors use a registered `custom` handler id + params. New units = new data rows in `shared/content/`, requiring no new code. Only 3 of the 26 reference engines need a handler (see spec §8 mapping table). Guard this ratio: keep the escape-hatch rare and isolated so the data model doesn't erode.
