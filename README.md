...
behavior uses a registered `custom` handler. Two handlers exist
(`replayAdjacentDeathrattle`, `primeNextDeathrattleDouble`). New units = new rows in
`shared/content/units.ts`.
